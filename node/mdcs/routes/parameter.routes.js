...
const newParameterApprovalHistoryPath = path.join(
  __dirname,
  '../data/new-parameter-approve-history.json'
);
const newParameterApprovalTempPath = path.join(
  __dirname,
  '../data/new-parameter-approve-temp.json'
);
...
router.post('/parameter/trial-history/search', async (req, res) => {
  try {
    const db = await readData(newParameterApprovalHistoryPath);
    const tempItems = await readData(newParameterApprovalTempPath);
    // const items = [...db]
    let data = [...db];
    ...
    if (processCount === 10) {
      ...
      const items = await readData(newParameterApprovePath);
      tempItems.forEach(item => {
        if (!items.some(d => d.param_master_id === item.param_master_id)) {
          items.unshift({ ...item, history_id: Date.now() });
        }
      })
      await saveData(items, newParameterApprovePath);
      await saveData([], newParameterApprovalTempPath);
...
router.post('/parameter/trial/live', async (req, res) => {
  try {
    ...
    await saveData(filtered, newParameterApprovePath);
    await saveData(tempItems, newParameterApprovalTempPath);
