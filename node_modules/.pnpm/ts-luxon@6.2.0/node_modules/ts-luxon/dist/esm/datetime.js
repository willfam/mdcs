...
import { isDefined, isUndefined, maybeArray, isDate, isNumber, bestBy, daysInMonth, daysInYear, isLeapYear, weeksInWeekYear, normalizeObject, roundTo, objToLocalTS, padStart, PLURAL_MAPPING } from "./impl/util.js";
...
import { gregorianToWeek, weekToGregorian, gregorianToOrdinal, ordinalToGregorian, hasInvalidGregorianData, hasInvalidWeekData, hasInvalidOrdinalData, hasInvalidTimeData, usesLocalWeekValues } from "./impl/conversions.js";
...
class DateTime {
  ...
  constructor(config) {
    ...
    let invalid = config.invalid || // invalid timestamp can happen when using plus or minus with 1E8 days resulting in overflows
    (Number.isNaN(config.ts) ? new Invalid("invalid timestamp") : null) || (!zone.isValid ? DateTime._unsupportedZone(zone) : null);
  ...
  static fromObject(obj = {}, opts = {}) {
    ...
    const tsNow = (_a = opts.overrideNow) !== null && _a !== void 0 ? _a : Settings.now();
    const offsetProvis = isNumber(opts.specificOffset) ? opts.specificOffset : zoneToUse.offset(tsNow);
  ...
  static _buildObject(config, units, defaultValues, objNow) {
    ...
    const higherOrderInvalid = config.useWeekData ? hasInvalidWeekData(config.normalized) : config.containsOrdinal ? hasInvalidOrdinalData(config.normalized) : hasInvalidGregorianData(config.normalized);
    const invalid = higherOrderInvalid || hasInvalidTimeData(config.normalized);
    if (invalid) {
      return DateTime.invalid(invalid);
    }
    const gregorian = config.useWeekData ? weekToGregorian(config.normalized, config.minDaysInFirstWeek, config.startOfWeek) : config.containsOrdinal ? ordinalToGregorian(config.normalized) : config.normalized, [tsFinal, offsetFinal] = objToTS(gregorian, config.offsetProvis, config.zoneToUse), inst = new DateTime({
      ts: tsFinal,
   ...
   * @return {string|null}
   */
  toISODate({ format = "extended", precision = "day" } = { format: "extended", precision: "day" }) {
    if (!this.isValid) {
      return null;
    }
