...
export class ParameterFileExportComponent implements OnInit, OnDestroy {
        ...
        .subscribe({
          next: (value: PayloadResponse) => {
            if (value.status == 200) {
              ...
              // Check if all items have reached a final status (SUCCESS or FAILED)
              // This allows a mix - some items can be SUCCESS while others are FAILED
              const processed =
                Array.isArray(exportEntities) &&
                exportEntities.length > 0 &&
                exportEntities.every((item: IParameterFileExportEntity) => {
              ...
              if (processed && !this.isParameterReadDialogShown) {
                this.stopStatusPolling();
                this.parameterReady();
              }
            }
  ...
  intervalStatus(): void {
    if (
      !(
        Array.isArray(this.storedExportData) && this.storedExportData.length > 0
      )
    ) {
      this.stopStatusPolling();
      return;
  ...
  */
  private cleanupAfterDownload(): void {
    ...
    // Clear grp_identifier_id from search filter
  ...
  updateDataSource(payload: any): void {
    ...
    this.dataSource = payload['param_file_export_entity_pgn'].map(
      (item: IParameterFileExportEntity) => {
        const mapped = this.mapData(item);

        const matchedStored =
          this.storedExportData &&
          this.storedExportData.find(
            x =>
              x.param_file_id === item.param_file_id &&
              x.param_payload_version === item.param_payload_version &&
              x.param_depot_id === item.param_depot_id
          );
        const isLoading = matchedStored?.status === 'PENDING';

        return {
          ...mapped,
          status: matchedStored?.status ?? item.status,
          isLoading: isLoading,
        };
      }
    );
    ...
    // Sort selected items to the top if there are selected items
  ...
  private showDownloadFailedDialog(): void {
    ...
    dialogRef
      ...
      .pipe(takeUntil(this.destroy$))
      .subscribe(() => {
        // Clear grp_identifier_id filter and refresh data
        // this.grpIdentifierIds = [];
        // this.params.search_select_filter = {
        //   ...this.params.search_select_filter,
        //   grp_identifier_id: [],
