...
export class ParameterFileImportComponent implements OnInit, OnDestroy {
  ...
  importHandler(event: any) {
    if (event.target.files.length) {
      ...
      const dialogRef = this.dialog.open(
        FileImportConfirmationDialogComponent,
        {
          width: '576px',
          height: '244px',
          disableClose: true,
      ...
      dialogRef.afterClosed().subscribe((confirmed: boolean) => {
        if (confirmed) {
          ...
          for (let index = 0; index < fileList.length; index++) {
            ...
            //TODO: temporary comment for now, will be open again for the new enhancement later
            // formData.append('depotId', this.depotSelected.toString());
          }
          this.importExportService
            .import(formData)
            .pipe(takeUntil(this.destroy$))
            .subscribe({
              next: (value: PayloadResponse) => {
                if (value.status == 201) {
                  // Extract grp_identifier_id from import response
                  ...
                  // Store unique grp_identifier_id values and update params
                  if (grpIdentifierIds.length > 0) {
                    const uniqueGrpIds = [...new Set(grpIdentifierIds)];
                    this.grpIdentifierIds = [
                      ...new Set([...this.grpIdentifierIds, ...uniqueGrpIds]),
                    ];
                    this.params.search_select_filter = {
                      ...this.params.search_select_filter,
                      grp_identifier_id: this.grpIdentifierIds,
                  ...
                  // Reset to page 1 to see the newly imported item
                  this.paginationService.currentPage = 1;
                  this.params.page_index = 0;
                  this.currentPage = 1;

                  // Start polling immediately after import since we know a file was just imported
                  // The newly imported item might not be in the current page results yet
          ...
          // User clicked No - reset file input
          if (this.fileInputRef?.nativeElement) {
            this.fileInputRef.nativeElement.value = '';
          }
        }
  ...
  private startStatusPolling(): void {
    ...
    // Don't start if no depot is selected
    if (!this.depotSelected) {
      return;
    }
