...
export class ViewCardKeyVersionComponent implements OnInit, OnDestroy {
  ...
  colDefs: (ColDef | ColGroupDef)[] = [
    ...
    },
    {
      ...
      headerClass: 'ag-center-header',
      headerGroupComponent: SortableHeaderGroupComponent,
      ...
      children: [
        {
          ...
          valueFormatter: (params: ValueFormatterParams) => {
            const date = new Date(params.value.value);
  ...
  icons: {
    sortAscending: '<div class="custom-sort"></div>',
    sortDescending: '<div class="custom-sort"></div>',
    sortUnSort: '<div class="custom-sort"></div>',
  };
  ...
  gridOptions: GridOptions = {
    ...
    defaultColDef: {
      flex: 1,
      minWidth: window.innerWidth > 992 ? undefined : 100,
      filter: false,
      sortable: false,
      resizable: true,
    },
 ...
 updateDataSource(payload: any): void {
  ...
  onGridReady(params: GridReadyEvent) {
    ...
    // Update minWidth based on viewport size
    this.resizeListener = () => {
      if (this.gridApi) {
        const minWidth = window.innerWidth > 992 ? undefined : 100;
        console.log('Viewport resized, updating column minWidth to:', minWidth);
  ...
  onSortChanged() {
    ...
    // Get sort model from ag-grid using getColumnState
    const columnState = this.gridApi.getColumnState();
    const sortedColumn = columnState.find((col: any) => col.sort !== null && col.sort !== undefined);
    
    if (sortedColumn && sortedColumn.sort) {
      this.params.sort_order = [
        {
          name: sortedColumn.colId,
